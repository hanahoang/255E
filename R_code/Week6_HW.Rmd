...
} #end of NOAAfunction definition
```
```{r}
...
Create a plot function to plot the TideHT and TimeUTC variables.


```{r plotFunction, echo= TRUE, message= FALSE, warning= FALSE}
# Create a plotting function for the tide data
plotTides <- function(df){
  plot(x= df$TimeUTC, y= df$TideHT, type= "l",
       xlab= "Date", ylab= "Tide Ht,m",
       xaxt= "n", col = "blue",
       main = "Tide Heigth v. Date")
  axis(side = 1, at=pretty(df$TimeUTC), labels= format(pretty(df$TimeUTC), format= "%b-%d-%Y" ))
}

```

## Collect data for 3 different months

### January 

```{r januarydata, cache= FALSE}
mo1 = NOAAfunction(9413450, 20080101, 20080131)

```
### February

```{r februarydata, cache= FALSE}
mo2 = NOAAfunction(9413450, 20080201, 20080229)
```
### March

```{r marchdata, cache= FALSE}
mo3 = NOAAfunction(9413450, 20080301, 20080328)
```
## Plot the 3 month

```{r}
moAll= rbind(mo1, mo2, mo3)
